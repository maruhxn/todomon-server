...
public class TodomonOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        Member member = memberReader.findOptionalByEmail(oAuth2ProviderUser.getEmail())
                .orElseGet(() -> memberWriter.registerByOAuth2(oAuth2ProviderUser));
        log.info("소셜 로그인 === 유저 아이디: {}, 유저명: {}, 이메일: {}", member.getId(), member.getUsername(), member.getEmail());
